...
# Read bdiffZuBsBasis
s_bdiffZuBs <- "bdiffZuBsBasisAlleMerkmale_ym0.csv"
tbl_bdiff <- readr::read_delim(file = s_bdiffZuBs, delim = ";")

tbl_nrr <- tbl_result %>% filter(Trait == "nrr")
## Join the tbl_result tibble with tbl_bdiff tibble
tbl_res_diff <- tbl_nrr %>% left_join(tbl_bdiff, by = c("Breed" = "rasse", "Trait" = "merkmal"))

#https://stackoverflow.com/questions/24459752/can-dplyr-package-be-used-for-conditional-mutating
tbl_res_diff %>% mutate(mean_zw_basis = if )


if(!is.na(tbl_res_diff$basisdifferenz)){
  tbl_res_diff$mean_zw_basis <- tbl_res_diff$mean_zw + tbl_res_diff$basisdifferenz
}else{
  tbl_res_diff$mean_zw_basis <- tbl_res_diff$mean_zw
}
...
# EBV of which breed should be set up
EBVfromBreedToSetUp<- unique(tbl_bdiffZuBs$rasse)

tbl_bkorr_test <- tbl_test[tbl_test[["Breed"]] == EBVfromBreedToSetUp,]

#mean_zw, min_zw und max_zw sollen mit Rassenkonstante angepasst werden

# Write output
#readr::write_delim(tbl_result, delim =";",path = paste(Sys.Date(),sep d = "_","GeneticTrendForRShiny.csv"))
...
tbl_BS_nrr <- tbl_result %>% filter(Breed == "BV/BS") %>% filter(Trait == "nrr")
